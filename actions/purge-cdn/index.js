...
// ── 入口 ──────────────────────────────────────────────
if (!REPO) {
    console.error('无法确定仓库名：请在 GitHub Actions 中运行，或设置 PURGE_REPO 环境变量（owner/repo）。');
    process.exit(1);
}
...
if (!afterSha) {
    try {
        ...
        console.error('用法: node actions/purge-cdn/index.js [before_sha] [after_sha]\n' +
...
function getChangedFiles(before, after) {
    const isNull = !before || before === 'null' || before === '0000000000000000000000000000000000000000';
    ...
    if (!isNull && !commitExists(base)) {
        console.warn(`⚠️ before commit ${base} 仍不可达（可能是 force push 覆盖了旧历史），降级为刷全部文件。`);
        base = EMPTY_TREE;
    }
    ...
    for (let i = 0; i < files.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                console.log(`::warning::第 ${batchIndex} 批 purge 返回 ${resp.status}，可能被限流，12h 内分支缓存也会自动过期。`);
            }
        }
        catch (err) {
            console.log(`::warning::第 ${batchIndex} 批 purge 网络错误: ${err instanceof Error ? err.message : err}`);
        }
    }
}
