 ...
 */
const IGNORED_DIRS = new Set<string>(['.git', 'node_modules', '.venv'])

// ── 类型 ──────────────────────────────────────────────

/** byDir 中单个目录节点：仅该层直接子项的 basename */
...
interface FsIndex {
	allFiles: string[]
	byDir: Record<string, DirListing>
}
 ...
 */
async function buildIndex(): Promise<FsIndex> {
	const allFiles: string[] = []
	const byDir: Record<string, DirListing> = {}
	...
	const sortedByDir: Record<string, DirListing> = {}
...
async function main(): Promise<void> {
	...
	console.log(
		`生成完成：${index.allFiles.length} 个文件、${dirCount} 个目录 → ${OUTPUT_NAME}`,
	)
}
